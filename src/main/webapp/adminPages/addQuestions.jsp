...
<body>
    ...
    <div class="container mt-4 p-4 shadow rounded bg-white">
        ...
        <form id="questionForm" action="/MindGauge/AddQuestion" method="post">
            <% 
            ...
            <% 
                ...
                } else response.sendRedirect("/MindGauge/error.jsp");
            %>
        ...
        <%
            } else response.sendRedirect("/MindGauge/adminPages/adminLogin.jsp");
        %>
    </div>
    ...
    <script>
        ...
        document.getElementById('deleteTestButton').addEventListener('click', function(event) {
            if (confirm('Current test will not be added. Are you sure you want to go back?')) {
                window.location.href = "/MindGauge/DeleteTest?what=test&test_id=" + event.target.dataset.testId;
            }
        });
