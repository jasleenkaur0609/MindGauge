    	...
    	if(request.getParameter("test_tag") != null) {
    		...
    		try {
    			if (questions == null || pass_marks == null || Integer.parseInt(pass_marks) < 0 || Integer.parseInt(pass_marks) > Integer.parseInt(questions)) {
    				response.sendRedirect("/MindGauge/adminPages/addTest.jsp?error=invalid_passing_marks&ques="+questions);
    				return;
    			}
    			if(TestDao.addNewTest(test_tag, questions, pass_marks, lang, topic, level)){
    				int test_id = TestDao.getTestId();
    				if(test_id == -1) {
    					response.sendRedirect("/MindGauge/adminPages/addTest.jsp?error=try_again");
    					return;
    				}
    			...
    			else response.sendRedirect("/MindGauge/adminPages/addTest.jsp?error=try_again");
