...
public class UserDao {
	...
	public static int getUserId(String mobile, String password) {
		...
		String query = "SELECT user_id FROM users WHERE mobile = ? AND password = ?";
	...
	public static boolean existsMobile(String mobile) {
		...
		String query = "SELECT username FROM users WHERE mobile = ? ";
		try(DbConnection db = new DbConnection();){
			db.pstm = db.con.prepareStatement(query);
			db.pstm.setString(1, mobile);
			db.rs = db.pstm.executeQuery();
	...
	public static boolean addUser(String username, String mobile, String password, String email) {
		String query = "INSERT INTO users (username, mobile, password, email) VALUES(?,?,?,?)";
		try(DbConnection db = new DbConnection()){
			db.pstm = db.con.prepareStatement(query);
	...
	public static String getUserName(String mobile) {
		...
		try(DbConnection db = new DbConnection();){
			...
			db.rs = db.pstm.executeQuery();
			String ans = "";
			while (db.rs.next()) ans = db.rs.getString("username");
			return ans;
		} 
		catch (Exception e) {
			e.printStackTrace();
	...
	public static String getPhone(int user_id) {
		...
		String query = "SELECT mobile FROM users WHERE user_id = ?";
		try(DbConnection db = new DbConnection();){
			db.pstm = db.con.prepareStatement(query);
