	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			try {
				...
				if(mobile.charAt(0) == '0') {
					response.sendRedirect("/MindGauge/userPages/userRegister.jsp?error=mobile_invalid");
					return;
				}
				
				// check if user with mobile already exists
				if(userDao.existsMobile(mobile)) {
					response.sendRedirect("/MindGauge/userPages/userRegister.jsp?error=user_already_exists");
					return;
				}
				
				if(userDao.existsEmail(email)) {
					response.sendRedirect("/MindGauge/userPages/userRegister.jsp?error=email_already_exists");
					return;	
				}
				...
				}else response.sendRedirect("/MindGauge/userPages/userRegister.jsp?error=try_again");
