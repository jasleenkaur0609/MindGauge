		...
		if(session != null) {
			try {
				...
				if(lang.equals("all") && topic.equals("all") && level.equals("all")) {
					if(person == "user") response.sendRedirect("/MindGauge/UserDashboard");
					if(person == "admin") response.sendRedirect("/MindGauge/AdminDashboard");
				}
			...
			response.sendRedirect("/MindGauge/error.jsp");
		}
	}
